...
namespace Sprdef.Tools.MemoryVisualizer
{
    public class ScreenPainter
    {
        ...
        public void Paint(Graphics g, Memory memory, Size clientSize, int yOffset, int rowCount, int columnCount, ScreenCharacterMap characters)
        {
            ...
            var innerClient = new Rectangle(outerClient.Left + borderWidth, outerClient.Top + borderHeight,
                outerClient.Width - borderWidth - borderWidth, outerClient.Height - borderHeight - borderHeight);
            ...
            using (var blue = new SolidBrush(Palette.GetColor(C64Color.Blue)))
            {
                ...
                g.FillRectangle(lightBlue, outerClient.Left, outerClient.Top + outerClient.Height - borderHeight,
                    outerClient.Width, borderHeight);
                g.FillRectangle(lightBlue, outerClient.Left + outerClient.Width - borderWidth, 0, borderWidth,
                    outerClient.Height);
                ...
                else
                {
                    ...
                    for (var y = 0; y < rowCount; y++)
                    {
                        ...
                        {
                            g.DrawString(characters.GetCharacter(x, y).ToString(), GridFont, lightBlue,
                                xPos + fontXOffset, yPos + fontYOffset);
                            xPos += characterWidth;
                        }
